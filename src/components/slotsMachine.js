...
const scriptHash = "51002b7a02f9b5a291fb5835b774ed349df6be42"
...
class Vote extends React.Component {
  ...
  */
    ...
    function spin(num1, num2, num3) {
      ...
      if (contains(nex[0], pick1 + 1)) {
        picked.push(1);
      } else if (contains(nos[0], pick1 + 1)) {
        picked.push(2);
      } else if (contains(neo[0], pick1 + 1)) {
        picked.push(3);
      } else if (contains(lx[0], pick1 + 1)) {
        picked.push(4);
      } else if (contains(neon[0], pick1 + 1)) {
        picked.push(5);
      } else if (contains(coz[0], pick1 + 1)) {
        picked.push(6);
      }
  
      if (contains(nex[1], pick2 + 1)) {
        picked.push(1);
      } else if (contains(nos[1], pick2 + 1)) {
        picked.push(2);
      } else if (contains(neo[1], pick2 + 1)) {
        picked.push(3);
      } else if (contains(lx[1], pick2 + 1)) {
        picked.push(4);
      } else if (contains(neon[1], pick2 + 1)) {
        picked.push(5);
      } else if (contains(coz[1], pick2 + 1)) {
        picked.push(6);
      }
  
      if (contains(nex[2], pick3 + 1)) {
        picked.push(1);
      } else if (contains(nos[2], pick3 + 1)) {
        picked.push(2);
      } else if (contains(neo[2], pick3 + 1)) {
        picked.push(3);
      } else if (contains(lx[2], pick3 + 1)) {
        picked.push(4);
      } else if (contains(neon[2], pick3 + 1)) {
        picked.push(5);
      } else if (contains(coz[2], pick3 + 1)) {
        picked.push(6);
      }
    ...
    $("#spin").on("click", async function(e) {
        e.preventDefault();
        var salt = Math.floor(Math.random() * 1000000000)
        var operation = "GenerateRandom"
        var args = [salt]
        var invoke = { scriptHash, operation, args };
        var result = null
        try {
            result = await props.nos.getStorage({scriptHash, key: "random", encodeInput: true, decodeOutput: false})
            result = parseInt(u.reverseHex(result), 16) * salt
        }
        catch(e) {
            // ignore
            console.log("have to generate from tx")
        }
        var txid = await props.nos.invoke(invoke)
        if (result === null) {
            result = 1
            for (var i=0; i<3; i++) {
                var random_place = Math.floor(Math.random() * (txid.length + 1));
                result = result * txid.charCodeAt(random_place)          
            }
        }
        var n1 = result % 10
        var n2 = ((result - n1) / 10) % 10
        var n3 = parseInt((result % 1000) / 100)
